...
import sys
import llama_index
from llama_index import (VectorStoreIndex,SimpleDirectoryReader,load_index_from_storage,StorageContext,LLMPredictor, PromptHelper, ServiceContext)
...
def create_index(directory, unique_folder_id):
    ...
    documents, storage_context=chroma_storage_context, service_context=service_context
    )
...
def ask_question(query, unique_folder_id):
    ...
    storage_context = StorageContext.from_defaults(persist_dir=dynamic_storage_context)
    # load index
    index = llama_index.indices.loading.load_index_from_storage(storage_context, index_id=dynamic_vector_id)
    while True:
        ...
        print(response)
        return str("Node ID: " + response.source_nodes[0].node_id + "\nLast Modified: " 
                   + str(response.source_nodes[0].metadata.get('last_modified_date')) + "\n"
                   + "Chatbot Response: \n" 
                   + response.response + "\n")
